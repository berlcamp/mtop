-- Lists the body and plate numbers that more than one ACTIVE franchise is
-- using, so they can be resolved before
-- 20260413000021_unique_unit_identifiers.sql is applied.
--
-- Read-only. Unlike duplicate operators, a duplicate body or plate number has
...
--
-- Migration 21's helper does not exist yet at this point, so the normalisation
